...
function setTimer(minutes) {
  chrome.runtime.sendMessage({ type: "setTimer", duration: minutes }, (response) => {
    if (response && response.status === "timerSet") {
      ...
      document.getElementById('yt-limiter-popup').style.display = 'none';
    }
  });
}
...
function checkTimeAndUpdateUsage() {
  ...
  if (window.location.hostname.includes("youtube.com")) {
    chrome.runtime.sendMessage({ type: "checkTime" }, (response) => {
      if (response) {
        if (response.block) {
          window.location.href = chrome.runtime.getURL("blocked.html");
        } else if (!response.timerRunning) {
          injectPopup();
          document.getElementById('yt-limiter-popup').style.display = 'flex';
        } else {
        ...
        const remainingTimeElement = document.getElementById('yt-limiter-remaining-time');
        if (remainingTimeElement) {
          ...
          timeDisplay.id = 'yt-limiter-remaining-time';
